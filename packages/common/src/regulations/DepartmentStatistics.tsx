...
import { availabilityLabels, canonicalDepartment, organizationSnapshot, validPublicUrl, type DepartmentResidualLabelRow, type DepartmentResidualOccurrenceRow, type PublishNoticeRow, type PublishRegulationRow } from "./index";

type Sort = "NONPUBLIC_RATE" | "NONPUBLIC_COUNT" | "NOTICE_COUNT" | "FULLTEXT_RATE";
type Props = { rows: PublishRegulationRow[]; notices: PublishNoticeRow[]; residuals:DepartmentResidualOccurrenceRow[]; residualLabels:DepartmentResidualLabelRow[] };

export function DepartmentStatistics({ rows, notices, residuals, residualLabels }: Props) {
  ...
  return <main className="shell statistics-page"><p className="eyebrow">부서별 통계</p><h1>사규예고 담당부서 현황</h1><p className="statistics-note">공식 조직도 명칭과 정확히 일치하는 사규예고 담당부서만 집계합니다. 전문 미공개율은 연결 규정 중 전문공개가 아닌 규정의 비율입니다.</p>
    ...
    <DepartmentResidualAnalysis occurrences={residuals} summary={residualLabels}/>
  </main>;
}
